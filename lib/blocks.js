...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix whivecore to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.dashd.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new whivecore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in whivecore-node.
      var header = whivecore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [whivecore.Transaction().fromBufferReader(br)];
